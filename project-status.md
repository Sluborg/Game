...
- **Side-facing + monster look (2026-06-21):** units now render the LPC **left** pose and the hero is **CSS-mirrored** to face right (most held-weapon layers only have left frames, so this keeps the weapon visible both ways). Weapons are dagger / arming / **mace** (all side-capable; longsword dropped — no side frames). Goblins/orcs gained **long ears** (LPC `head/ears/long`, skin-tinted) so they read as goblinoid rather than green humans. HP bar visibility fixed: the numeric readout is now its own line above a width-constrained bar (no longer overlapping the sprite). Real goblin/orc/troll *bodies* still aren't in this repo (only skeleton/zombie exist) — flagged in `ASSET_SOURCES.md`.
